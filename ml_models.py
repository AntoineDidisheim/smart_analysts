...
class MultipleLayerAggregator(LinearAggregator):
    def __init__(self,input_size, nb_pred_standing, layer_width, layer_types=[tf.nn.relu], name='unnamed_model_low_rate', summary_type ="", rate_of_saving=20, verbose=True,start_rate=0.001):
        ...
        with tf.name_scope('aggregation'):
            ...
            self.omega = tf.add(tf.nn.relu(tf.multiply(tf.transpose(tf.reduce_sum(finalOutput, axis=(2))),self.non_zero_check)),self.finalConstants,name='omega')
...
if __name__ == '__main__':
    ...
    model = MultipleLayerAggregator(start_rate = 1, input_size=5, nb_pred_standing=4, rate_of_saving=1,layer_types=[tf.nn.sigmoid,tf.nn.relu],layer_width=[5000,5000],name='2large',summary_type='simple_simulation')
